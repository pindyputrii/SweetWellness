...
const Admin = () => {
  ...
  const [selectedRecipe, setSelectedRecipe] = useState(null); // STATE UNTUK MODAL
  const navigate = useNavigate();

  // State Form Tambah Resep
  const [newRecipe, setNewRecipe] = useState({
    title: "", desc: "", category: "Kue", calories: "", image: "", ingredients: "", instructions: ""
  });
        ...
        try {
          ...
          if (userDoc.exists() && userDoc.data().role === "admin") {
            ...
            const unsubRecipes = onSnapshot(q, (snapshot) => {
              ...
              // Sortir agar Pending ada di paling atas
              data.sort((a, b) => {
  ...
  const handleAddRecipeSubmit = async (e) => {
    ...
    try {
      ...
      setNewRecipe({ title: "", desc: "", category: "Kue", calories: "", image: "", ingredients: "", instructions: "" });
  ...
  const getCategoryStyle = (cat) => {
    ...
    if (lower.includes("gluten free")) return "bg-orange-100 text-orange-700";
    if (lower.includes("kue")) return "bg-red-100 text-red-700";
    return "bg-gray-100 text-gray-600";
  };

  const filteredRecipes = recipes.filter(r => selectedCategory === "Semua" ? true : r.category?.toLowerCase().includes(selectedCategory.toLowerCase()));
  
  // LOGIC APPROVE / REJECT (Dengan Tutup Modal)
  ...
  return (
    <div className="flex min-h-screen bg-[#FFF5F5] font-sans">
        ...
        <header className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-3xl font-bold text-[#4B110D]">
              {activeTab === 'dashboard' ? 'Dashboard Overview' : activeTab === 'recipes' ? 'Kelola Resep' : activeTab === 'add' ? 'Tambah Resep Baru' : activeTab === 'users' ? 'Kelola Pengguna' : 'Statistik & Analitik'}
            </h2>
           ...
           <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden p-6">
             <div className="flex gap-2 mb-6">
                <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="border rounded-lg px-3 py-2 bg-gray-50 text-sm">
                  <option value="Semua">Semua Kategori</option><option value="Low Carb">Low Carb</option><option value="High Fiber">High Fiber</option><option value="Kue">Kue</option>
                </select>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-4xl">
            ...
            <form onSubmit={handleAddRecipeSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div><label className="block text-sm font-medium mb-1">Kategori</label><select className="w-full border rounded-lg p-3" value={newRecipe.category} onChange={e => setNewRecipe({...newRecipe, category: e.target.value})}><option>Kue</option><option>Puding</option><option>Es Krim</option></select></div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div><label className="block text-sm font-medium mb-1">Bahan</label><textarea rows="5" className="w-full border rounded-lg p-3" value={newRecipe.ingredients} onChange={e => setNewRecipe({...newRecipe, ingredients: e.target.value})}></textarea></div>
                <div><label className="block text-sm font-medium mb-1">Cara Membuat</label><textarea rows="5" className="w-full border rounded-lg p-3" value={newRecipe.instructions} onChange={e => setNewRecipe({...newRecipe, instructions: e.target.value})}></textarea></div>
              </div>
        ...
        {activeTab === 'stats' && (
          <div className="space-y-8">
             <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                <StatCardNew label="Total Views" value={`${(stats.totalViews / 1000).toFixed(1)}k`} sub="Total Views" growth="+35%" icon={<Icons.Stats/>} color="bg-red-100 text-red-600" />
                <StatCardNew label="Pengguna Baru" value={stats.totalUsers} sub="Pengguna Baru" growth="+12%" icon={<Icons.Users/>} color="bg-red-100 text-red-600" />
                <StatCardNew label="Total Likes" value={stats.totalLikes} sub="Total Likes" growth="+28%" icon={<Icons.Heart/>} color="bg-red-100 text-red-600" />
                <StatCardNew label="Total Shares" value={stats.totalShares} sub="Total Shares" growth="+22%" icon={<Icons.Share/>} color="bg-red-100 text-red-600" />
             </div>
      ...
      {/* --- MODAL DETAIL / PREVIEW RESEP (FIXED: SPLIT ERROR) --- */}
      {selectedRecipe && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto relative">
            ...
            <div className="h-64 bg-gray-200 w-full relative">
               ...
               {selectedRecipe.status === 'pending' && <span className="absolute bottom-4 left-4 bg-yellow-400 text-black px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest shadow-md">Menunggu Review</span>}
            </div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                  <div>
                     ...
                     <ul className="text-sm text-gray-600 space-y-2 list-disc pl-5">
                        {/* SAFE RENDER: Mencegah Error .split() */}
                        {(() => {
    ...
    <table className="w-full text-left border-collapse">
      ...
      <tbody className="divide-y divide-gray-100 text-sm">
        {data.length === 0 ? <tr><td colSpan="4" className="text-center py-8 text-gray-400">Tidak ada data.</td></tr> : 
          data.map((recipe) => (
            <tr key={recipe.id} className={`hover:bg-gray-50 ${recipe.status === 'pending' ? 'bg-yellow-50' : ''}`}>
              ...
              <td className="px-6 py-4 text-center flex justify-center gap-2">
                ...
                {recipe.status === 'pending' ? (<><button onClick={() => handleApprove(recipe.id, recipe.title)} className="p-2 bg-green-50 text-green-600 rounded hover:bg-green-100"><Icons.Check /></button><button onClick={() => handleReject(recipe.id, recipe.title)} className="p-2 bg-yellow-50 text-yellow-600 rounded hover:bg-yellow-100"><Icons.X /></button></>) : (<button onClick={() => handleDelete(recipe.id)} className="p-2 bg-gray-50 text-gray-400 rounded hover:text-red-500"><Icons.Trash /></button>)}
              </td>
            </tr>
        ))}
      </tbody>
